...
import DailyOfferForm from "../DailyOfferForm/DailyOfferForm";

const DailyShifts = ({ dateStr }) => {
    ...
    const currentDayShifts = shifts.filter((shift) => (shift.date === dateStr && shift.swap === "dailySwap"))
    // if(currentDayShifts.length) console.log(currentDayShifts.map((shift)=>shift._id))  //first API fetch may have enpty []

    const offerShifts = shifts.filter((shift) => (shift.offerTo === clickShift._id && shift.offer === "dailyOffer"))
            ...
            <div className='Shifts'>
                ...
                >
                    ...
                    <DialogTitle id="alert-dialog-title" sx={{ paddingTop: "0", pb: "0" }}>
                        <p className="ShiftSwap-formTitle" style={{ color: "#f73378" }}>For Daily Offer</p>
                    </DialogTitle>
                    <DialogContent
                        sx={{ minHeight: "200px" }}
                    >
